//CODIGO AQUI

let saldo = 15

saldo = saldo + 22.40 
// saldo += 22.40
console.log(saldo);

saldo = saldo - 12.34
//saldo -= 12.34
console.log(saldo);

saldo = saldo / 2
//saldo /= 2
console.log(saldo);

saldo = saldo * 3.4
//saldo *= 3.4
console.log(saldo);

//módulo
saldo = saldo % 3
//saldo %= 3
console.log(saldo);

// ctrl + ; = vc comenta os códigos
//--------------------------------


let operacao = 2 * 2 + 2 //= 6
// 2 * (2 + 2) = 8
console.log(operacao);


const comparacao = 5 === 5 //true
console.log(comparacao);

const comparacao2 = 5 !== 5 //false
console.log(comparacao2);

const comparacao3 = 5 !== "5" //true
console.log(comparacao3);

const comparacao4 = "5" === "cinco" //false
console.log(comparacao4);

const comparacao5 = typeof 5 === typeof 20 //true
console.log(comparacao5);

const comparacao6 = typeof "5" ===typeof "cinco" //true
console.log(comparacao6);
// !== tipo e valor
// -------------------------------

const primeiroNumero = 57
const segundoNumero = 13

const verificacao = primeiroNumero === segundoNumero //false
console.log(verificacao);

const verificacao2 = primeiroNumero <= segundoNumero //false
console.log(verificacao2);

const verificacao3 = primeiroNumero > segundoNumero //true
console.log(verificacao3);

const verificacao4 = primeiroNumero < segundoNumero //false
console.log(verificacao4);

//--------------------------------